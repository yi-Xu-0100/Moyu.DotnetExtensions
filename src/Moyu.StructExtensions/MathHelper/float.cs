...
{
    /// <summary>
    /// 判断两个 <see cref="float"/> 值是否在指定误差范围内相等。
    /// </summary>
    ...
    public static bool IsEqual(this float a, float b, float epsilon = 1e-6f) => Math.Abs(a - b) < epsilon;

    /// <summary>
    /// 返回指定 <see cref="float"/> 值的符号，考虑给定的误差范围。
    /// </summary>
